...
#case if p flag was passed
if [ $p_flag -eq 1 ]; then
    #get passwd
    echo  $PASSWD_PROMPT
    read -s passwd

    #Main loop to get arguments after p flag
    while [ ${#} -gt 0 ]; do

	#If Argument is file then encrypt it with passwd
	if [ -f $1 ]; then
	    dir="$(dirname $1)"
	    mkdir -p $dir/.secret
	...
	#If argument is a directory then encrypt it with passwd
	elif [ -d $1 ]; then
	    dir="$(dirname $1)"
	...
	#case if file of directory does not exists
	else 
	    echo $FILE_ERR $1
	fi
...
#case if no flag was passed
elif [ $u_flag -eq 0 ]; then

    #Main loop to get arguments
    while [ ${#} -gt 0 ]; do
	
	#If Argument is file then encrypt it
	if [ -f $1 ]; then
	    dir="$(dirname $1)"
            mkdir -p $dir/.secret
            zip -q $1.zip $1
	...
	#If argument is a directory then encrypt it
	elif [ -d $1 ]; then
	    dir="$(dirname $1)"
	    zip -qr $1.zip $1
	    rm -r $1
...
#-------------------------------------------------------------------------------
#Case if u-flag was passed
if [ $u_flag -eq 1 ]; then
    u_path=$1
    
    #checks that the argument is a directory and contains a .secret directory
    if [ -d $u_path/$SECRET_DIR ]; then
	
	#Special case if the directory's .secret directory has a .secret.zip to unzip
	unzip -q $u_path/$SECRET_DIR/.secret.zip 2> /dev/null
	if [ $? -eq 0 ]; then
	    rm $u_path/$SECRET_DIR/.secret.zip

	#Else unzip all files and directories within the .secret directory in the u_path
	else
    ...
    #Case if no .secret directory found
    else
